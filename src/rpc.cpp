...
namespace hpgc {
    ...
    void RPCNetwork::RegisterCallback(int req_type, Message * req, Message * resp,
                                      Callback cb) {
    ...
    void RPCNetwork::Send(RPCRequest * req) {
        req->start_time = Now();
        req->mpi_req = m_world->Issend(req->payload.data(), req->payload.size(),
                                       MPI::BYTE, req->target, req->rpc_type);
