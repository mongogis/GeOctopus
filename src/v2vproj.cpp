...
namespace hpgc{
	...
	{
		RegisterVector();
		OGRDataSource * poSourceDs = VectorOpen(barrel->GetDataSource().c_str(),GA_ReadOnly);
		ON_SCOPE_EXIT([&](){OGRDataSource::DestroyDataSource(poSourceDs); });

		OGRDataSource * poOutputDs = VectorOpen(barrel->GetDataSource().c_str(), GA_Update);
		ON_SCOPE_EXIT([&](){OGRDataSource::DestroyDataSource(poOutputDs); });

		OGRLayer * poSrcLayer = poSourceDs->GetLayerByName(barrel->GetLayer().c_str());
		OGRLayer * poDstLayer = poOutputDs->GetLayerByName(barrel->GetLayer().c_str());
		OGRSpatialReference *poSourceSRS = poSrcLayer->GetSpatialRef();
		OGRCoordinateTransformation *poCT = poCT = OGRCreateCoordinateTransformation(poSourceSRS,m_ogrSr);

		OGRFeatureDefn * poDstFeatureDefn = poDstLayer->GetLayerDefn();

		std::for_each(begin(barrel->GetFeatures()),end(barrel->GetFeatures()), [&](int fid){
			...
			OGRGeometry* poReprojectedGeom = OGRGeometryFactory::transformWithOptions(poDstGeometry, poCT, NULL);
